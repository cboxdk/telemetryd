//! Decoding a Prometheus remote-read response.
//!
//! Lives here rather than in the CLI for one reason: it is a hand-written protobuf
//! decoder reading bytes off a network, which is the same class of code as
//! `remote_write` next door — and that one shipped a defect where eight bytes could ask
//! for a 2.8 GB allocation. A parser in a binary crate cannot be fuzzed; one here can,
//! and now is.
//!
//! Field numbers are from `prompb/remote.proto` and `prompb/types.proto`:
//!
...
pub fn read_request(start_ms: i64, end_ms: i64) -> Vec<u8> {
...
pub type Series = (Vec<(String, String)>, Vec<(i64, f64)>);

pub fn parse_response(body: &[u8]) -> Result<Vec<Series>, telemetryd_core::Error> {
    let mut out = Vec::new();
...
fn parse_series(series: &mut Reader<'_>) -> Result<Series, telemetryd_core::Error> {
...
pub fn to_otlp(series: &[Series]) -> (Value, u64) {
